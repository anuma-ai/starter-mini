...
test("demo walkthrough", async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    const container = input.closest('[data-testid*="otp"]') || input.parentElement?.parentElement;
    if (!container) return;
  ...
  // then wait for streamdown content to appear and the button to switch back.
